  ...
  return (
    <div className="container mx-auto px-4 sm:px-6 lg:px-8 max-w-5xl py-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-4 mb-8">
        {[
          { icon: Car, label: 'Total Booking', value: bookings.length },
          { icon: Calendar, label: 'Aktif', value: bookings.filter((b) => b.status === 'confirmed').length },
          { icon: TrendingUp, label: 'Total Pengeluaran', value: formatCurrency(totalSpend) },
        ].map((stat) => (
      ...
      <div className="flex gap-1 mb-6 bg-slate-100 dark:bg-slate-800 p-1 rounded-xl w-fit">
        {(['bookings', 'profile'] as const).map((tab) => (
          ...
          >
            {tab === 'bookings' ? 'Riwayat Booking' : 'Edit Profil'}
          </button>
        ))}
      </div>
        ...
        <div>
          {bookingsLoading ? (
            ...
            <div className="text-center py-16 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800">
              ...
              <h3 className="font-semibold text-slate-900 dark:text-white mb-1">Belum ada booking</h3>
              <p className="text-slate-500 dark:text-slate-400 text-sm mb-5">Mulai sewa mobil impianmu sekarang</p>
              <a
                href="/cars"
